...
import { TRANSLATIONS, formatDuration, isWarmup, isCooldown, normalizeRoutine } from '../../../utils/helpers.js';
...
const MatrixValue = ({ value, unit, subtext, label }) => {
    ...
    return (
        <div className="relative w-[4.3rem] h-[4.3rem] xs:w-20 xs:h-20 bg-slate-900/90 rounded-2xl border border-slate-700/50 p-2 flex flex-col items-center justify-center shrink-0 shadow-lg backdrop-blur-sm pointer-events-none select-none overflow-hidden">
            {label && <span className="absolute top-0 left-0 bg-slate-800/80 text-[8px] font-black text-slate-400 px-1.5 py-0.5 rounded-br-lg border-r border-b border-slate-700/50 z-10">{label}</span>}
...
const AdjustableLoadMatrix = ({ initialLoad, initialUnit, onUpdate, label }) => {
    ...
    useEffect(() => { setLoad(initialLoad ?? 0); }, [initialLoad]);

    const handleTouchStart = (e) => { e.stopPropagation(); lastUpdateY.current = e.touches[0].clientY; setIsInteracting(true); };
    const handleTouchMove = (e) => {
        ...
        if (Math.abs(deltaY) > 25) {
            ...
            setLoad(newLoad); onUpdate(newLoad); lastUpdateY.current = currentY;
        }
    };
    
    let displayUnit = initialUnit || "KG";
    let displayValue = load;
    if ((load === 0 && displayUnit.toUpperCase() === 'KG') || displayUnit.toUpperCase() === 'BW' || displayUnit.toUpperCase() === 'BODYWEIGHT') { displayUnit = "Peso"; displayValue = "BW"; }

    const containerClass = `relative w-[4.3rem] h-[4.3rem] xs:w-20 xs:h-20 rounded-2xl border flex flex-col items-center justify-center shrink-0 transition-all duration-200 cursor-ns-resize select-none touch-none overflow-hidden ${isInteracting ? 'bg-teal-900/80 border-teal-500/50 scale-105 shadow-[0_0_20px_rgba(20,184,166,0.3)] z-10' : 'bg-slate-900/90 border-slate-700/50 shadow-lg backdrop-blur-sm'}`;
    const textClass = isInteracting ? 'text-teal-400' : 'text-white';
    const unitClass = isInteracting ? 'text-teal-500/70' : 'text-slate-500';

    return (
        <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={(e) => { e.stopPropagation(); setIsInteracting(false); }} onClick={(e) => e.stopPropagation()} className={containerClass}>
            {label && <span className={`absolute top-0 left-0 text-[8px] font-black px-1.5 py-0.5 rounded-br-lg border-r border-b z-20 ${label.includes('A1') ? 'bg-cyan-900/40 text-cyan-400 border-cyan-500/20' : label.includes('A2') ? 'bg-blue-900/40 text-blue-400 border-blue-500/20' : 'bg-slate-800/80 text-slate-400 border-slate-700/50'}`}>{label}</span>}
...
export const ActiveSession = ({ currentRoutine, handleRoutineFeedback, lang, onExerciseComplete, restSeconds, setRestSeconds, setIsSessionActive, isSessionActive, sessionSeconds, handleBackToMain, title }) => {
    ...
    const warmupBlocks = normalizedBlocks.filter(b => b.fase_sesion === 'warmup');
    const cooldownBlocks = normalizedBlocks.filter(b => b.fase_sesion === 'cooldown');
    const workoutBlocks = normalizedBlocks.filter(b => b.fase_sesion === 'main');
    ...
    const nextBlock = phase === 'workout' && blockIndex + 1 < workoutBlocks.length ? workoutBlocks[blockIndex + 1] : (phase === 'workout' && cooldownBlocks.length > 0 ? cooldownBlocks[0] : null);
    ...
    const handleLoadUpdate = (bIdx, sIdx, itemIdx, newLoad) => setUserLoads(prev => ({ ...prev, [`${bIdx}-${sIdx}-${itemIdx}`]: newLoad }));
        ...
        if (isNowDone) {
            ...
            if (activeBlock && activeBlock.items) {
                activeBlock.items.forEach((item, itemIdx) => {
                    setData[`exercise_${itemIdx}`] = { ejercicio: item.ejercicio, reps: item.reps_target, load: userLoads[`${blockIndex}-${setIndex}-${itemIdx}`] ?? item.peso_valor };
                });
            }
    ...
    const handleNext = () => {
        if (phase === 'warmup') {
            if (workoutBlocks.length > 0) { setPhase('workout'); setBlockIndex(0); }
            else if (cooldownBlocks.length > 0) setPhase('cooldown');
            else handleRoutineFeedback?.(routineId, { sets: completedSets }, "", "completed");
        } else if (phase === 'workout') {
            if (blockIndex < workoutBlocks.length - 1) setBlockIndex(prev => prev + 1); 
            else if (cooldownBlocks.length > 0) setPhase('cooldown');
            else handleRoutineFeedback?.(routineId, { sets: completedSets }, "", "completed");
    ...
    const formatWarmupList = (items) => items.map((item, i) => (
        <li key={i} className="flex items-start gap-3 text-left">
            <span className="w-1.5 h-1.5 rounded-full bg-orange-700 mt-2 shrink-0"></span>
            <span className="text-slate-300 text-xl leading-relaxed font-medium">{item.ejercicio || item.nombre}</span>
        </li>
    ));

    return (
        <div className="h-screen w-full bg-black flex flex-col overflow-hidden relative selection:bg-teal-500/30">
            {/* FONDO ANIMADO CON ORBES ROJOS */}
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
                <div className="absolute top-[-10%] left-[-20%] w-[500px] h-[500px] bg-red-900/20 rounded-full blur-[120px] animate-pulse-slow"></div>
                <div className="absolute bottom-[-10%] right-[-20%] w-[400px] h-[400px] bg-rose-900/15 rounded-full blur-[100px] animate-pulse-slow delay-1000"></div>
                <div className="absolute top-[40%] left-[30%] w-[300px] h-[300px] bg-red-600/5 rounded-full blur-[80px] animate-pulse"></div>
            </div>
            ...
            <div className="relative z-10 flex flex-col h-full">
                <header className="shrink-0 px-4 pt-6 pb-2 bg-gradient-to-b from-black/80 to-transparent z-40 flex items-center gap-4">
                    <button onClick={handleBackToMain} className="p-3 bg-slate-900/50 border border-slate-800 rounded-full text-slate-400 active:scale-90 hover:text-white transition-all shadow-lg shrink-0"><Icon name="arrowLeft" className="w-5 h-5" /></button>
                    <div className="flex-1 flex flex-col justify-center min-w-0">
                        <div className="flex justify-between items-baseline mb-1.5">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest truncate max-w-[70%]">{phase === 'warmup' ? t.warmupTitle : phase === 'cooldown' ? t.cooldownTitle : activeBlock?.items[0]?.ejercicio || `BLOQUE ${blockIndex + 1}`}</span>
                            <span className="text-[10px] font-black text-teal-400 tabular-nums">{`${Math.round(progressPercent)}%`}</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-800/50 rounded-full overflow-hidden border border-slate-700/30"><div className="h-full bg-gradient-to-r from-teal-500 to-emerald-400 shadow-[0_0_8px_rgba(20,184,166,0.6)] transition-all duration-700 ease-out rounded-full" style={{ width: `${progressPercent}%` }}></div></div>
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-32 minimal-scrollbar pt-2">
                    {phase === 'workout' && activeBlock ? (
                                    ...
                                    <div key={idx} className={idx > 0 ? "mt-4 pt-4 border-t border-white/5" : ""}>
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                {isSuperset && <span className={`shrink-0 text-[10px] font-black px-2 py-0.5 rounded border ${idx === 0 ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' : 'bg-blue-500/20 text-blue-400 border-blue-500/30'}`}>{`A${idx + 1}`}</span>}
                        ...
                        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
                            <div className={`w-20 h-20 rounded-3xl ${phase === 'warmup' ? 'bg-orange-500/10' : 'bg-blue-500/10'} flex items-center justify-center mb-6 animate-pulse`}><Icon name={phase === 'warmup' ? "flame" : "wind"} className={`w-10 h-10 ${phase === 'warmup' ? 'text-orange-500' : 'text-blue-400'}`}/></div>
                            ...
                            <div className="bg-slate-900/60 border border-white/5 rounded-[2rem] p-6 mb-10 w-full max-w-sm backdrop-blur-md">
                                <ul className="space-y-4">
                                    {phase === 'warmup' && activeWarmupItems.length > 0 ? formatWarmupList(activeWarmupItems) : phase === 'cooldown' && activeCooldownItems.length > 0 ? formatWarmupList(activeCooldownItems) : <li className="text-slate-400 text-sm italic">Prepárate para la sesión</li>}
                                </ul>
                            </div>
                        </div>
                    )}
                    ...
                    {phase === 'workout' ? (
                        <div className="pointer-events-auto flex items-center gap-6 bg-slate-900/90 border border-white/10 rounded-full p-2 pr-6 pl-4 backdrop-blur-xl shadow-2xl">
                            ...
                            <div className="flex items-center gap-4">
                                <button onClick={() => setIsSessionActive(!isSessionActive)} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-90 ${!isSessionActive ? 'bg-amber-500 text-white shadow-[0_0_15px_rgba(245,158,11,0.5)]' : 'bg-slate-800 text-slate-400 border border-white/5'}`}><Icon name={!isSessionActive ? "play" : "pause"} className="w-5 h-5 fill-current"/></button>
                                <div className="text-lg font-mono font-black text-teal-400 tracking-tighter tabular-nums min-w-[3.5rem] text-center">{formatDuration(sessionSeconds)}</div>
                            </div>
                    ...
                    <div className="absolute inset-0 z-[60] bg-slate-950/98 flex flex-col animate-fadeIn backdrop-blur-3xl p-6 text-center">
                        ...
                        <div className="w-full max-w-sm mx-auto shrink-0 pb-8">
                            <div className="bg-black/20 rounded-2xl p-4 mb-6 text-left">
                                ...
                                <div className="text-base font-bold text-white bg-slate-800/40 p-4 rounded-lg truncate">{nextBlock ? nextBlock.items.map(i => i.ejercicio).join(" + ") : t.cooldownTitle || 'Enfriamiento'}</div>
                            </div>
                            <div className="space-y-4">
